...
const EqAppConvolutionEq = (props) => {
  ...
  const onDownloadClick = () => {
    ...
    if (stereo) {
      ...
      audioBuffer = props.audioContext.createBuffer(
        2,
        props.firAudioBuffer.length,
        props.firAudioBuffer.sampleRate
      );
  ...
  if (props.firAudioBuffer) {
    ...
    isFirDecodeFailed =
      firData.filter((x) => x === 0.0).length === firData.length;
  }
  ...
  return (
    <Grid container direction="column" rowSpacing={1}>
      {props.instructions && (
        ...
        >
          <Typography variant="caption">{props.instructions}</Typography>
        </Grid>
      )}
      <Grid item container direction="row" columnSpacing={1}>
        ...
        <Grid item xs={4} sx={{ position: "relative" }}>
          <Autocomplete
            renderInput={(params) => (
              <TextField {...params} label="Bit depth" />
            )}
      ...
      {isFirDecodeFailed && (
        <Grid item>
          <Typography variant="caption" color="error">
            Failed to decode filter audio data. Try another browser.
          </Typography>
        </Grid>
      )}
      <Grid item>
        <InputSlider
          ...
          initialValue={props.fRes}
          min={1.0}
          max={40.0}
          step={1.0}
          onChange={(v) => {
            props.onEqParamChanged({ fRes: v });
          }}
      ...
      <Grid item>
        <InputSlider
          label="Preamp"
          initialValue={props.preamp}
          min={-20}
          max={20}
          step={0.5}
          onChange={(v) => {
            props.onEqParamChanged({ preamp: v });
          }}
          tooltip="Apply negative preamp if you hear clipping. SonicSync normalizes convolution filters but cannot guarantee the audio won't ever clip."
        />
      </Grid>
      <Grid
        item
      ...
      >
        ...
        >
          ...
          <Grid item>
            ...
            >
              <InfoOutlinedIcon
                sx={{ width: 18, height: 18, verticalAlign: "bottom" }}
              />
            </Tooltip>
          </Grid>
        </Grid>
