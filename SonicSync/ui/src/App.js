...
const index_to_genre = {
  blues: <FaBluesky />,
  metal: <LuHandMetal />,
  classical: <GiClassicalKnowledge />,
  country: <MdHolidayVillage />,
  rock: <FaGuitar />,
  hiphop: <GiBilledCap />,
  reggae: <GiFlute />,
  pop: <LuPartyPopper />,
  jazz: <GiAvoidance />,
  disco: <PiDiscoBallDuotone />,
};
...
const App = (props) => {
  ...
  const [selectedMeasurement, setSelectedMeasurement, selectedMeasurementRef] =
    useStateRef(null); // { label, source, form, rig }
  ...
  const [preferredTargets, setPreferredTargets, preferredTargetsRef] =
    useStateRef([]);
  // Name (label) of the currently selected target.
  const [selectedTarget, setSelectedTarget, selectedTargetRef] =
    useStateRef(null);
  const [soundSignature, setSoundSignature, soundSignatureRef] =
    useStateRef(null); // Sound signature { frequency, raw }
  // Smoothing window size for sound signature
  const [
    soundSignatureSmoothingWindowSize,
    setSoundSignatureSmoothingWindowSize,
    soundSignatureSmoothingWindowSizeRef,
  ] = useStateRef(1.0);

  const [equalizers, setEqualizers, equalizersRef] =
    useStateRef(defaultEqualizers);
  const [selectedEqualizer, setSelectedEqualizer, selectedEqualizerRef] =
    useStateRef(null); // Name of the selected equalizer app
  ...
  const setState = {
    ...
    soundSignatureSmoothingWindowSize: (v) => {
      soundSignatureSmoothingWindowSizeRef.current = v;
    },
    bassBoostFc: (v) => {
      bassBoostFcRef.current = v;
    },
    bassBoostQ: (v) => {
      bassBoostQRef.current = v;
    },
    bassBoostGain: (v) => {
      bassBoostGainRef.current = v;
    },
    trebleBoostFc: (v) => {
      trebleBoostFcRef.current = v;
    },
    trebleBoostQ: (v) => {
      trebleBoostQRef.current = v;
    },
    trebleBoostGain: (v) => {
      trebleBoostGainRef.current = v;
    },
    tilt: (v) => {
      tiltRef.current = v;
    },
    ...
    maxGain: (v) => {
      maxGainRef.current = v;
    },
    windowSize: (v) => {
      windowSizeRef.current = v;
    },
    trebleWindowSize: (v) => {
      trebleWindowSizeRef.current = v;
    },
    trebleFLower: (v) => {
      trebleFLowerRef.current = v;
    },
    trebleFUpper: (v) => {
      trebleFUpperRef.current = v;
    },
    trebleGainK: (v) => {
      trebleGainKRef.current = v;
    },
    smoothed: setSmoothed,
    maxSlope: (v) => {
      maxSlopeRef.current = v;
    },
  };
  ...
  const equalize = async (skipTimer) => {
    ...
    if (!skipTimer) {
      equalizeTimerRef.current = setTimeout(() => {
        equalize(true);
      }, 500);
      return;
    }

    const selectedEqualizerObj = find(
      equalizers,
      (equalizer) => equalizer.label === selectedEqualizerRef.current
    );
    ...
    const eqParams = {
      ...
      equalizer: find(
        equalizersRef.current,
        (eq) => eq.label === selectedEqualizerRef.current
      ),
      target: target,
      soundSignature: soundSignatureRef.current,
      soundSignatureSmoothingWindowSize:
        soundSignatureSmoothingWindowSizeRef.current,
  ...
  const onSoundProfileSelected = (name) => {
    const newState = cloneDeep(
      find(soundProfilesRef.current, (p) => p.name === name)
    );
    ...
    const newlySelectedSoundProfile = cloneDeep(
      find(soundProfilesRef.current, (p) => p.name === name)
    );
  ...
  const onSoundProfileCreated = () => {
    ...
    newSoundProfile.name = newSoundProfiles.length
      ? Math.max(...newSoundProfiles.map((p) => parseInt(p.name))) + 1
      : 1;
    newSoundProfiles.push(newSoundProfile);
    window.localStorage.setItem(
      "soundProfiles",
      JSON.stringify(newSoundProfiles)
    );
  ...
  const onTargetSelected = (target) => {
    ...
    if (
      typeof newPreferredTarget[selectedMeasurementRef.current.source][
        selectedMeasurementRef.current.form
      ] !== "string"
    ) {
      newPreferredTarget[selectedMeasurementRef.current.source][
        selectedMeasurementRef.current.form
      ][selectedMeasurementRef.current.rig] = target.label;
    } else {
      newPreferredTarget[selectedMeasurementRef.current.source][
        selectedMeasurementRef.current.form
      ] = target.label;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Grid
            ...
            xs={12}
            xxl={6}
            sx={{ display: !!graphData && !showInfo ? "block" : "none" }}
            order={{ xxl: 1 }}
          >
            {/* <div>Hello world</div> */}
            {models && (
              ...
              >
                ...
                >
                  <h1>{index_to_genre[models.model1]}</h1>
                  <Typography variant="body2">{models.model1}</Typography>
                </div>
                <div
                  style={{
                    backgroundColor: "white",
                ...
                >
                  <h1>{index_to_genre[models.model2]}</h1>
                  <Typography variant="body2">{models.model2}</Typography>
                </div>
              </div>
            )}
            <SmPaper
      ...
      <Box
        sx={{ position: "fixed", bottom: { xs: 0, sm: 8 }, left: 0, right: 0 }}
      >
        <Container fixed maxWidth="sm" sx={{ padding: "0 !important" }}>
          <Player
            ...
            isEqEnabled={
              eqNodesRef.current.length > 0 && selectedEqualizer !== null
            }
          />
        </Container>
      </Box>
