...
const Waves = (props) => {
  ...
  const sine = (wavelength, amplitude, y) => {
    ...
    return `M 0 ${y} c ${cx} ${-cy}, ${
      wavelength - cx
    } ${cy}, ${wavelength} 0 c ${cx} ${-cy}, ${
      wavelength - cx
    } ${cy}, ${wavelength} 0 v 100 h ${-2 * wavelength} Z`;
  };
  ...
  const createPaths = () => {
    ...
    for (let i = 0; i < props.nWaves; ++i) {
      const swellDuration =
        12 + (((i / props.nWaves) * 1.5) ** 2 - (3 * i) / props.nWaves) * 6;
      const swellDelay = -props.nWaves + i * 0.3;
      const waveDuration = 15;
      const waveDelay =
        (((i / props.nWaves) * 1.5) ** 2 - (3 * i) / props.nWaves) * 6;
      paths.push(
        <svg
          key={i}
          style={{
            animation: `swell ${swellDuration}s ease ${swellDelay}s infinite`,
          }}
        >
          <path
