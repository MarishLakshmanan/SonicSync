...
const ListboxComponent = forwardRef(function ListboxComponent(props, ref) {
  const {
    children,
    showOnlyUniqueLabels,
    onShowOnlyUniqueLabelsChanged,
    ...other
  } = props;
  const itemData = [];
            ...
            <ButtonGroup variant="text">
              <Button
                ...
                sx={{
                  height: 22,
                  textTransform: "none",
                  color: !showOnlyUniqueLabels ? "primary" : "gray",
                }}
...
const PopperComponent = (props) => {
  return (
    <Popper
      {...props}
      style={{ width: 500, maxWidth: "calc(100vw - 20px)" }}
      placement="bottom"
    />
  );
};
...
const CSVAutocomplete = (props) => {
  ...
  if (props.showOnlyUniqueLabels) {
    ...
    for (const option of props.options) {
      if (
        options.length === 0 ||
        options[options.length - 1].label !== option.label
      ) {
        options.push(option);
      }
    }
      ...
      {props.useSelect && (
        <FormControl fullWidth>
          ...
          <Select
            ...
            onChange={(e) => {
              props.onChange(
                find(props.options, (option) => option.label === e.target.value)
              );
            }}
            value={props.value}
        ...
        props.virtualize !== false && (
          <Autocomplete
            ...
            ListboxProps={{
              ...
              onShowOnlyUniqueLabelsChanged:
                props.onShowOnlyUniqueLabelsChanged,
            }}
            PopperComponent={PopperComponent}
            renderOption={(liProps, option) => [
              liProps,
              option,
              props.renderOption || null,
            ]}
            value={props.value}
            options={options}
            isOptionEqualToValue={(option, value) =>
              option.label === value.label && isEqual(option, value)
            }
            onChange={(e, val) => {
              props.onChange(val);
            }}
